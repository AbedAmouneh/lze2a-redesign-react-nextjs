  ...
  const getFeaturedArray = (product) => {
    ...
    for (let i = 0; i < products.length; i++) {
      for (let j = 0; j < products[i].length; j++) {
        if (
          products[i][j].id != product.id &&
          products[i][j].category == product.category
        ) {
  ...
  return (
    <>
          ...
          <div className="Product-Buttons">
            <div className="Number-Selector" style={borderStyle}>
              <button
                onClick={() => {
                  setSelectedNumber(
                    selectedNumber == 0 ? 0 : selectedNumber - 1,
                  );
                }}
              >
                -
            ...
            <button className="Add-to-Cart" style={addToCartStyle}>
              Add to Cart
            </button>
            <button className="Customize-btn" style={addToCartStyle}>
              Customize
            </button>
          </div>
        </div>
      ...
      <div className="Featured-row">
        {getFeaturedArray(product).map((item, Index) => (
          <ProductItem
            key={Index}
            id={item.id}
            image={item.image}
